        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                scanLine = lerp(1.0, scanLine, _ScanLines);
                                float4 originalColor = tex2D(_MainTex, glitchedUV) * _BaseColor;
                
                float2 redUV = glitchedUV + float2(_ColorShift * sin(time * 3.0), 0.0);
                float2 blueUV = glitchedUV - float2(_ColorShift * cos(time * 2.0), 0.0);
                
                float4 redChannel = tex2D(_MainTex, redUV) * _BaseColor;
                float4 greenChannel = tex2D(_MainTex, glitchedUV) * _BaseColor;
                float4 blueChannel = tex2D(_MainTex, blueUV) * _BaseColor;

                float4 finalColor = float4(redChannel.r, greenChannel.g, blueChannel.b, originalColor.a);
